...
const CheckoutForm = () => {
 ...
 const elements=useElements()
  ...
  const createPaymetIntent=async()=>{
try {
  const {data}=await axios.post("/.netlify/functions/createPaymentIntent",JSON.stringify({
 ...
 createPaymetIntent()
  ...
  const handleChange=async (event)=>{
    ...
    setEroor(event.error?event.error.message:"")
  }
  const handleSubmit=async(ev)=>{
    ...
    if(payload.error){
      setEroor(`Payment failed ${payload.error.message}`)
      setProcessing(false)
    } else{
      setEroor(null)
      setProcessing(false)
...
<p className={succeeded?"result-message":"result-message hidden"}>
  payment succeeded, go to <a href={`https://dashboard.stripe.com/test/payments`}> dashboard </a>
</p>
    </form>
     </div>
}
